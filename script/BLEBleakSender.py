...
FILE_PATH = "Preprocessing/binary.txt"  # File path for the input data

# Changed: Function to read data from a txt file
def get_data_from_file(file_path):
    with open(file_path, 'r') as file:
        for line in file:
            yield line.strip().encode('utf-8')  # Encode each line as bytes

async def send_data():
    async with BleakClient(DEVICE_ADDRESS) as client:
        print(f"Connected: {client.is_connected}")

        # Changed: Generator initialization
